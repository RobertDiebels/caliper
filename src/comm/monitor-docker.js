    ...
    for (let h in filterName.remote) {
        ...
        let p = docker.listContainers().then((containers) => {
            ...
            if (this.filter.images) {
                for (let i = 0; i < size; i++) {
                    this.filter.images.forEach((imageName) => {
                        if (containers[i].Image.match(imageName) !== null) {
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        if (self.containers[i].remote === null) {    // local
                            ...
                            if (cpuDelta > 0.0 && sysDelta > 0.0) {
                                if ((stat.cpu_stats.cpu_usage.hasOwnProperty('online_cpus') || stat.cpu_stats.cpu_usage.hasOwnProperty('percpu_usage')) && stat.cpu_stats.cpu_usage.percpu_usage !== null) {
